...
def atualizar_analise(regioes_selecionadas, canais_selecionados, produtos_selecionados):
    ...
    fig_bar.update_traces(
        ...
        customdata=np.stack([
            df_bar["canal_vendas"], 
            df_bar["regiao"], 
            df_bar["lucro"].apply(formatar_brl),  # Lucro formatado
            df_bar["vendas"].apply(formatar_brl)  # Vendas formatado
        ], axis=-1),
    ...
    fig_scatter.update_traces(
        ...
        customdata=np.stack([
            ...
            df_filtrado["regiao"],
            df_filtrado["vendas"].apply(formatar_brl),  # Vendas formatado
            df_filtrado["lucro"].apply(formatar_brl),  # Lucro formatado
            df_filtrado["margem"]
        ], axis=-1)
    )
